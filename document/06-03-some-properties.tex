...
\item {\bf Non-equal, $\redex_0 \neq \redextwo$.}
  ...
  %:%and, moreover, if $\redex_0 \permle \redextwo\redseqthree$ then $\redex_0 \in \redextwo\redseqthree$.
  ...
  \item Secondly, we may check that $\redex_1$ is not $(\tm'/\redextwo)$-garbage.
        ...
        since $\redex_0 \neq \redextwo$, as a consequence of \rlem{different_redexes_have_disjoint_images}.
  \end{enumerate}
  Applying the \ih, we obtain that $\#(\redex_0/\redextwo\redseqthree) = \#(\redex_1/\redseqthree) \leq 1$.  
  %:%Moreover, suppose that $\redex_0 \permle \redseqtwo = \redextwo\redseqthree$.
  %:%Then $\redex_1 = \redex_0/\redextwo \permle \redseqthree$.
  %:%By \ih, $\redex_1 \in \redseqthree$, so $\redex_0 \in \redseqthree$, which concludes the proof.
\end{enumerate}
\end{proof}

\begin{lemma}[Prefix of garbage]
\llem{prefix_of_garbage}
...
then by \rcoro{simulation_residuals_and_prefixes}.
we have $\redseq/\tm' \permle \redseqtwo/\tm' = \emptyDerivation$,
so $\redseq/\tm'$ is garbage, as required.
\end{proof}

\begin{lemma}[Composition of garbage]
\llem{composition_of_garbage_is_garbage}
...
\begin{lemma}[Projection of garbage]
\llem{projection_of_garbage_is_garbage}
...
\begin{lemma}[Join of garbage]
\llem{join_of_garbage_is_garbage}
...
that the composition of garbage is garbage~(\rlem{composition_of_garbage_is_garbage}),
...
by \rlem{contexts_refined_by_terms_may_be_filled}.
...
\item $(2 \implies 3)$
  ...
  Then
  by \rlem{composition_of_garbage_is_garbage}
...
Moreover, $\names(\redex/\tm')$ and $\names(\redextwo/\tm')$ are disjoint
since $\redex \neq \redextwo$, by \rlem{different_redexes_have_disjoint_images}.
...
\item $(3 \implies 1)$
  ...
  \item {\bf If there exists a coarse step for $(\redseqtwo,\tm')$.}
    ...
    \[
      \left(\frac{\redex_0}{\redseqthree}\right) \left(\frac{(\redseqtwo/\redex_0) \sieve (\tm'/\redex_0)}{\redseqthree/\redex_0}\right)
    \]
    Recall that, in general, $AB$ is garbage if and only if $A$ and $B$ are garbage (\rlem{composition_of_garbage_is_garbage}).
    ...
    \item {\bf (A)}
      ...
      %By the generalized cube lemma for simulation residuals~\rlem{generalized_cube_lemma},
      %this is equivalent to saying that $(\redex_0/\tm')/(\redseqthree/\tm') = \emptyset$.
      %On the other hand, since $\redex_0$ is coarse for $(\redseqtwo,\tm')$, we know that $\redex_0/\tm' \neq \emptyset$.
...
\item {\bf If there are no coarse steps for $(\redseq,\tm')$.}
  ...
  Recall that the composition of garbage is again garbage (\rlem{composition_of_garbage_is_garbage}),
